...
let selfCalendarMonth=currentMonth();
...
function getOvertimeRequestPay(request){
  const employee = emp(request.emp);
  if(!employee) return 0;
  const hours = Number(request.hours);
  if(!Number.isFinite(hours) || hours <= 0) return 0;
  const multiplier = isWeekend(request.date) ? 2 : 1.5;
  return hours * getSalaryBreakdown(employee).hourly * multiplier;
}
function getExceptionTimeText(exception){
  const attendance=db.attendance.find(record=>record.emp===exception.emp&&record.date===exception.date);
  const inTime=exception.requestedIn||exception.in||attendance?.in;
  const outTime=exception.requestedOut||exception.out||attendance?.out;
  return `Time: ${formatTime(inTime)} - ${formatTime(outTime)}`;
}
...
function getLeaveAccrual(employee, year=todayISO().slice(0,4)){
 const annualEntitlement=Number(employee.annualLeaveDays??30),workingDays=Math.max(1,Math.min(7,Number(db.settings?.workingDays)||5))*52,workingMinutes=getEmployeeWorkingMinutes(employee),minutesPerFullDay=annualEntitlement*workingMinutes/workingDays;
 const qualifyingDays=db.attendance.filter(record=>record.emp===employee.id&&record.date.startsWith(year)&&!isWeekend(record.date)&&getWorkedMinutes(record)>=workingMinutes).length;
 return {qualifyingDays,workingMinutes,accruedMinutes:Math.min(annualEntitlement*workingMinutes,qualifyingDays*minutesPerFullDay),minutesPerFullDay};
}
function getLeaveBalanceDetails(employee,type){
 const configuredBalance=Number(db.leaveBalances[employee.id]?.[type]||0),workingMinutes=getEmployeeWorkingMinutes(employee);
 if(type==='Annual'){
  const accrual=getLeaveAccrual(employee),usedMinutes=getLeaveUsage(employee.id,type)*workingMinutes,availableMinutes=Math.max(0,accrual.accruedMinutes-usedMinutes);
  return {...accrual,availableMinutes,availableDays:availableMinutes/workingMinutes};
 }
 const availableMinutes=Math.max(0,configuredBalance-getLeaveUsage(employee.id,type))*workingMinutes;
 return {qualifyingDays:0,workingMinutes,accruedMinutes:configuredBalance*workingMinutes,availableMinutes,availableDays:availableMinutes/workingMinutes};
}
...
function fillSelects(){
 const selectedEmpDept=document.getElementById('empDept').value,selectedPayDept=document.getElementById('payDept').value,selectedDashboardDept=document.getElementById('dashboardDept').value,selectedReportDept=document.getElementById('reportDept').value,selectedManagerDept=document.getElementById('managerDept')?.value||'';
 ...
 document.getElementById('empDept').value=selectedEmpDept;document.getElementById('payDept').value=selectedPayDept;document.getElementById('dashboardDept').value=selectedDashboardDept;document.getElementById('reportDept').value=selectedReportDept;if(document.getElementById('managerDept')){document.getElementById('managerDept').innerHTML='<option value="">All departments</option>'+db.departments.map(d=>`<option>${esc(d)}</option>`).join('');document.getElementById('managerDept').value=selectedManagerDept}
...
function renderManager(){
 const managerDept=document.getElementById('managerDept')?.value||'',activeEmployees=db.employees.filter(employee=>employee.status==='Active'&&(!managerDept||employee.dept===managerDept)),todayRecords=db.attendance.filter(record=>record.date===todayISO()&&activeEmployees.some(employee=>employee.id===record.emp)),presentToday=todayRecords.filter(record=>['Present','Late'].includes(record.status)).length,lateToday=todayRecords.filter(record=>record.status==='Late').length,approvedLeaveEmployees=new Set(db.leaves.filter(leave=>leave.status==='Approved'&&leave.from<=todayISO()&&leave.to>=todayISO()&&activeEmployees.some(employee=>employee.id===leave.emp)).map(leave=>leave.emp)),leaveToday=approvedLeaveEmployees.size,absentToday=Math.max(0,activeEmployees.length-presentToday-leaveToday),coverageRisks=activeEmployees.filter(employee=>{const record=todayRecords.find(item=>item.emp===employee.id);return !approvedLeaveEmployees.has(employee.id)&&(!record||!record.out||record.status==='Absent')});
 const attendanceRate=activeEmployees.length?Math.round(presentToday/activeEmployees.length*100):0;
 document.getElementById('managerPulseDate').textContent=todayISO();document.getElementById('managerPresentToday').textContent=presentToday;document.getElementById('managerLateToday').textContent=lateToday;document.getElementById('managerAbsentToday').textContent=absentToday;document.getElementById('managerLeaveToday').textContent=leaveToday;document.getElementById('managerAttendanceRate').textContent=`${attendanceRate}%`;document.getElementById('managerAttendanceProgress').style.width=`${attendanceRate}%`;document.getElementById('managerRiskCount').textContent=coverageRisks.length;
 document.getElementById('managerCoverageRisks').innerHTML=coverageRisks.length?`<div class="risk-list">${coverageRisks.slice(0,5).map(employee=>{const record=todayRecords.find(item=>item.emp===employee.id),label=record?.status==='Absent'?'Marked absent':record&&!record.out?'Missing checkout':'No check-in';return `<div class="risk-row"><div><b>${esc(employee.name)}</b><small>${esc(employee.dept)} · ${label}</small></div><button class="btn" onclick="openManagerAttendance('${esc(employee.id)}')">Review</button></div>`}).join('')}</div>${coverageRisks.length>5?`<div class="subtle risk-more">+${coverageRisks.length-5} more employees</div>`:''}`:'<div class="empty">All active employees have a complete attendance record.</div>';
 const pendingExceptions=db.manager.exceptionRequests.filter(x=>x.status==='Pending'&&(!managerDept||emp(x.emp)?.dept===managerDept));
 const pendingOvertime=db.manager.overtimeRequests.filter(x=>x.status==='Pending'&&(!managerDept||emp(x.emp)?.dept===managerDept));
 ...
 document.getElementById('managerExceptions').innerHTML=pendingExceptions.length?`<div class="review-list">${pendingExceptions.map(x=>`<div class="review-row"><div class="review-main"><b>${esc(emp(x.emp)?.name||x.emp)} · ${esc(x.type)}</b><small>${esc(x.date)} · ${getExceptionTimeText(x)} · ${esc(x.reason)}</small></div><div class="review-actions"><button class="btn success" onclick="decideException('${esc(x.id)}','Approved')">Approve</button><button class="btn danger" onclick="decideException('${esc(x.id)}','Rejected')">Reject</button></div></div>`).join('')}</div>`:'<div class="empty">No attendance exceptions need review.</div>';
 document.getElementById('managerOvertime').innerHTML=pendingOvertime.length?`<div class="review-list">${pendingOvertime.map(x=>`<div class="review-row"><div class="review-main"><b>${esc(emp(x.emp)?.name||x.emp)} · ${Number(x.hours).toFixed(2)} hrs</b><small>${esc(x.date)} · ${esc(x.reason)} · Amount: ${money(getOvertimeRequestPay(x))}</small></div><div class="review-actions"><button class="btn success" onclick="decideOvertime('${esc(x.id)}','Approved')">Approve</button><button class="btn danger" onclick="decideOvertime('${esc(x.id)}','Rejected')">Reject</button></div></div>`).join('')}</div>`:'<div class="empty">No overtime requests need review.</div>';
 document.getElementById('managerSchedule').innerHTML=`<div class="schedule-list">${activeEmployees.map(x=>`<div class="schedule-chip"><b>${esc(x.name)}</b><span>${esc(formatTime(x.startTime||'09:00'))} - ${esc(formatTime(x.endTime||'17:00'))} · ${Number(x.workingHours||8).toFixed(1)} hrs</span></div>`).join('')||'<div class="empty">No active employees.</div>'}</div>`;
 const summaries=activeEmployees.reduce((acc,x)=>{const summary=getEmployeeMonthlySummary(x,currentMonth());acc.salary+=summary.salary.total;acc.ot+=summary.totalOTPay;acc.paidLeave+=summary.leave.paidLeaveSalary;acc.unpaidLeave+=summary.leave.unpaidDeduction;acc.total+=summary.totalPay;return acc},{salary:0,ot:0,paidLeave:0,unpaidLeave:0,total:0});
...
function openManagerAttendance(employeeId){go('attendance');document.getElementById('attSearch').value=emp(employeeId)?.name||employeeId;document.getElementById('attDatePreset').value='today';document.getElementById('attStatus').value='';toggleDateInput('attDatePreset','attDateRange');syncDatePresetButtons('attDatePreset');renderAttendance()}
function renderApprovals(){
 ...
 document.getElementById('approvalExceptions').innerHTML=exceptions.length?`<div class="review-list">${exceptions.map(item=>`<div class="review-row"><div class="review-main"><b>${esc(emp(item.emp)?.name||item.emp)} · ${esc(item.type)}</b><small>${esc(item.date)} · ${getExceptionTimeText(item)} · ${esc(item.reason||'Review requested')}</small></div><div class="review-actions"><button class="btn success" onclick="decideException('${esc(item.id)}','Approved')">Approve</button><button class="btn danger" onclick="decideException('${esc(item.id)}','Rejected')">Reject</button></div></div>`).join('')}</div>`:'<div class="empty">No attendance approvals pending.</div>';
 ...
 document.getElementById('approvalOvertime').innerHTML=overtime.length?`<div class="review-list">${overtime.map(item=>`<div class="review-row"><div class="review-main"><b>${esc(emp(item.emp)?.name||item.emp)} · ${Number(item.hours).toFixed(2)} hours</b><small>${esc(item.date)} · ${esc(item.reason||'Overtime request')} · Amount: ${money(getOvertimeRequestPay(item))}</small></div><div class="review-actions"><button class="btn success" onclick="decideOvertime('${esc(item.id)}','Approved')">Approve</button><button class="btn danger" onclick="decideOvertime('${esc(item.id)}','Rejected')">Reject</button></div></div>`).join('')}</div>`:'<div class="empty">No overtime approvals pending.</div>';
}
...
function renderSelfService(){
 ...
 document.getElementById('selfProfileId').value=employee.id||'';document.getElementById('selfProfileName').value=employee.name||'';document.getElementById('selfProfileDept').value=employee.dept||'';document.getElementById('selfProfileEmail').value=employee.email||'';document.getElementById('selfProfileLocation').value=employee.location||'';document.getElementById('selfProfileStart').value=formatTime(employee.startTime||'09:00');document.getElementById('selfProfileEnd').value=formatTime(employee.endTime||'17:00');document.getElementById('selfProfileHours').value=`${Number(employee.workingHours??employee.workHours??8).toFixed(1)} hours`;document.getElementById('selfProfileSalary').value=money(getSalaryBreakdown(employee).total);document.getElementById('selfProfileStatus').value=employee.status||'';
 ...
 const leaveBalances=db.leaveBalances[employee.id]||{};const remainingLeave=Object.keys(leaveBalances).reduce((sum,type)=>sum+getLeaveBalanceDetails(employee,type).availableDays,0);
 document.getElementById('selfLeaveBalance').textContent=`${remainingLeave.toFixed(1)} days`;
 ...
 const balances=db.leaveBalances[employee.id]||{};document.getElementById('selfLeaveDetails').innerHTML=Object.keys(balances).map(type=>{const details=getLeaveBalanceDetails(employee,type),wholeDays=Math.floor(details.availableMinutes/details.workingMinutes),hoursRemainder=Math.floor((details.availableMinutes%details.workingMinutes)/60),minutesRemainder=Math.round(details.availableMinutes%60);return `<div class="payslip-line"><span>${esc(type)}${type==='Annual'?`<small>${details.qualifyingDays} full day(s) accrued</small>`:''}</span><b>${wholeDays} days left${type==='Annual'?` · ${hoursRemainder}h ${minutesRemainder}m accrued`:''}</b></div>`}).join('');
...
function renderPortalLeaveBalance(){const employee=getSelectedSelfEmployee();if(!employee)return;const balances=db.leaveBalances[employee.id]||{};document.getElementById('selfLeaveDetails').innerHTML=`<table class="table portal-table"><thead><tr><th>Leave type</th><th>Days</th><th>Hours</th><th>Minutes</th></tr></thead><tbody>${Object.keys(balances).map(type=>{const details=getLeaveBalanceDetails(employee,type);return `<tr><td>${esc(type)}</td><td>${details.availableDays.toFixed(1)} days</td><td>${(details.availableMinutes/60).toFixed(1)} hours</td><td>${details.availableMinutes.toFixed(1)} minutes</td></tr>`}).join('')}</tbody></table>`}
function renderSelfAttendanceCalendar(records){renderPortalLeaveBalance();const month=currentMonth(),first=new Date(`${month}-01T12:00:00`),days=new Date(first.getFullYear(),first.getMonth()+1,0).getDate(),byDate=Object.fromEntries(records.map(record=>[record.date,record]));let html='<div class="attendance-calendar">';for(let day=1;day<=days;day++){const date=`${month}-${String(day).padStart(2,'0')}`,record=byDate[date],status=record?.status?.toLowerCase()||'empty';html+=`<div class="calendar-day ${status}" title="${record?esc(record.status):'No record'}"><b>${day}</b><small>${record?esc(record.status.slice(0,1)):'-'}</small></div>`}document.getElementById('selfAttendanceCalendar').innerHTML=html+'</div>'}
...
function renderLeaveBalances(){
 document.getElementById('leaveBalances').innerHTML=db.employees.filter(x=>x.status==='Active').map(x=>{const annual=getLeaveBalanceDetails(x,'Annual'),wholeDays=Math.floor(annual.availableMinutes/annual.workingMinutes),hoursRemainder=Math.floor((annual.availableMinutes%annual.workingMinutes)/60),minutesRemainder=Math.round(annual.availableMinutes%60);return `<tr><td><b>${esc(x.name)}</b><br><small>${esc(x.id)}</small></td><td>${annual.qualifyingDays.toFixed(1)}</td><td>${annual.accruedMinutes.toFixed(1)}</td><td>${(annual.accruedMinutes/60).toFixed(1)}</td><td><b>${wholeDays.toFixed(1)}</b></td><td>${hoursRemainder.toFixed(1)}</td><td>${minutesRemainder.toFixed(1)}</td></tr>`}).join('')||'<tr><td colspan="7" class="empty">No active employees.</td></tr>';
}
...
function renderAll(){fillSelects();renderDashboard();renderManager();renderApprovals();renderSelfService();normalizeSelfPortalTables();renderPortalLeaveBalance();renderModernSelfCalendar(getSelfCalendarRecords());renderAttendance();renderEmployees();renderDepartments();renderShifts();renderWeeklySchedule();renderPayroll();renderLocations();renderLeaves();renderLeaveBalances();renderHolidayList();renderAudit();renderReports()}
...
setupTableActions();
document.getElementById('selfEmployee').onchange=()=>{selfCalendarMonth=currentMonth();renderSelfService();normalizeSelfPortalTables();renderModernSelfCalendar(getSelfCalendarRecords())};
renderAll();
normalizeSelfPortalTables();
renderModernSelfCalendar(getSelfCalendarRecords());
