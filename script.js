...
const sample=()=>({
 employees:[
  {id:'EMP-001',name:'Alex Morgan',dept:'Engineering',phone:'+971 50 111 2200',rate:28,status:'Active',bio:'Face + Fingerprint'},
  {id:'EMP-002',name:'Sara Khan',dept:'Finance',phone:'+971 50 222 3300',rate:24,status:'Active',bio:'Fingerprint'},
  {id:'EMP-003',name:'Omar Ali',dept:'Operations',phone:'+971 50 333 4400',rate:18,status:'Active',bio:'QR'}
 ],
...
function rate(r){return emp(r.emp)?.rate||0}
function pay(r){return hours(r)*rate(r)}
...
function openModal(id){fillSelects();document.getElementById(id).classList.add('show')}
...
function renderAttendance(){
 ...
 document.getElementById('attendanceBody').innerHTML=rows.map(a=>`<tr><td>${esc(a.date)}</td><td><b>${esc(emp(a.emp)?.name||a.emp)}</b><br><small>${esc(a.emp)}</small></td><td>${esc(a.method)}</td><td>${esc(a.in)}</td><td>${esc(a.out)}</td><td>${hours(a).toFixed(2)}</td><td>${money(rate(a))}</td><td><b>${money(pay(a))}</b></td><td>${esc(a.location||'—')}</td><td><span class="badge ${a.status.toLowerCase()}">${esc(a.status)}</span></td></tr>`).join('')||'<tr><td colspan="10" class="empty">No records match the selected filters.</td></tr>';
}
function renderEmployees(){
 const q=(document.getElementById('empSearch').value||'').toLowerCase(),d=document.getElementById('empDept').value;
 document.getElementById('employeesBody').innerHTML=db.employees.filter(e=>(!q||[e.id,e.name,e.dept,e.phone].join(' ').toLowerCase().includes(q))&&(!d||e.dept===d)).map(e=>`<tr><td>${esc(e.id)}</td><td><b>${esc(e.name)}</b></td><td>${esc(e.dept)}</td><td>${esc(e.phone)}</td><td>${money(e.rate)}</td><td>${esc(e.bio||'Not enrolled')}</td><td><span class="badge ${e.status==='Active'?'present':'absent'}">${esc(e.status)}</span></td><td><button class="btn" onclick="removeEmployee('${esc(e.id)}')">Delete</button></td></tr>`).join('')||'<tr><td colspan="8" class="empty">No employees found.</td></tr>';
}
...
function renderPayroll(){
 ...
 document.getElementById('payrollBody').innerHTML=db.employees.filter(e=>!dept||e.dept===dept).map(e=>{
  const rs=db.attendance.filter(a=>a.emp===e.id),today=rs.filter(a=>a.date===todayISO()).reduce((s,a)=>s+pay(a),0);
  const week=rs.filter(a=>{let d=new Date(a.date),n=new Date();return d>=new Date(n-6*86400000)&&d<=n}).reduce((s,a)=>s+pay(a),0);
  const mon=rs.filter(a=>a.date.startsWith(month)).reduce((s,a)=>s+pay(a),0);
  return `<tr><td><b>${esc(e.name)}</b></td><td>${esc(e.dept)}</td><td>${money(e.rate)}</td><td>${money(today)}</td><td>${money(week)}</td><td><b>${money(mon)}</b></td></tr>`}).join('');
}
...
function addEmployee(e){e.preventDefault();let x={id:eId.value.trim(),name:eName.value.trim(),dept:eDept.value,phone:ePhone.value.trim(),rate:+eRate.value,status:eStatus.value,bio:'Not enrolled'};if(db.employees.some(v=>v.id===x.id)){toast('Employee ID already exists');return}db.employees.push(x);addAudit('Employee added',x.name);save();closeModal('employeeModal');e.target.reset();renderAll();toast('Employee saved')}
