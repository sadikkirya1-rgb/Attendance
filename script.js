...
function setDatePreset(presetId,rangeId,value,render,button){document.getElementById(presetId).value=value;document.querySelectorAll(`[data-preset-target="${presetId}"] .preset-btn`).forEach(item=>item.classList.toggle('active',item===button));toggleDateInput(presetId,rangeId);render()}
...
const rolePages={admin:['dashboard','manager','selfservice','attendance','employees','departments','shifts','scanner','payroll','locations','leaves','reports','audit','settings'],manager:['dashboard','manager','selfservice','attendance','employees','shifts','locations','leaves','reports'],payroll:['dashboard','manager','selfservice','attendance','payroll','reports'],employee:['selfservice']};
...
function fillSelects(){
 const selectedEmpDept=document.getElementById('empDept').value,selectedPayDept=document.getElementById('payDept').value,selectedDashboardDept=document.getElementById('dashboardDept').value;
...
function renderDashboard(){
 ...
 document.getElementById('statEmployees').textContent=visibleEmployees.length;
 document.getElementById('statPresent').textContent=present;document.getElementById('presentRate').textContent=visibleEmployees.length?Math.round(present/visibleEmployees.length*100)+'%':'0%';
...
function approveLeave(i){if(!canPerform(['admin','manager']))return;db.leaves[i].status='Approved';addAudit('Leave approved',emp(db.leaves[i].emp)?.name||db.leaves[i].emp);save();renderAll();toast('Leave approved')}
function rejectLeave(i){if(!canPerform(['admin','manager']))return;db.leaves[i].status='Rejected';addAudit('Leave rejected',emp(db.leaves[i].emp)?.name||db.leaves[i].emp);save();renderAll();toast('Leave rejected')}
function clearAudit(){if(confirm('Clear audit log?')){db.audit=[];save();renderAudit();toast('Audit log cleared')}}
function clearFilters(){attSearch.value='';attDateFrom.value='';attDateTo.value='';attDatePreset.value='all';attStatus.value='';attPeriod.value='day';toggleDateInput('attDatePreset','attDateRange');syncDatePresetButtons('attDatePreset');renderAttendance()}
