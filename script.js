...
function renderReports(){const rows=getReportRows();document.getElementById('reportRecords').textContent=rows.length;document.getElementById('reportHours').textContent=rows.reduce((sum,record)=>sum+hours(record),0).toFixed(1);document.getElementById('reportPay').textContent=money(rows.reduce((sum,record)=>sum+pay(record),0));document.getElementById('reportLocations').textContent=new Set(rows.map(record=>record.location).filter(Boolean)).size;document.getElementById('reportBody').innerHTML=rows.map(record=>`<tr><td>${esc(record.date)}</td><td><b>${esc(emp(record.emp)?.name||record.emp)}</b></td><td>${esc(emp(record.emp)?.dept||'—')}</td><td><span class="badge ${record.status.toLowerCase()}">${esc(record.status)}</span></td><td>${hours(record).toFixed(2)}</td><td><b>${money(pay(record))}</b></td><td>${esc(record.location||'—')}</td></tr>`).join('')||'<tr><td colspan="7" class="empty">No report data matches the selected filters.</td></tr>'}
