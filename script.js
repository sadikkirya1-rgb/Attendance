...
function addEmployee(e){e.preventDefault();if(!canPerform(['admin','manager']))return;
  ...
  if(existing){
  Object.assign(existing,{name,dept,phone,rate,status,workingHours,workHours:workingHours,startTime,endTime,basicSalary:basicSalary||0,allowance,monthlySalary:monthlySalary || basicSalary + allowance,monthlyBonus,monthlyDeduction});
   addAudit('Employee updated',name);
  }else{
  const x={id,name,dept,phone,rate,status,bio:'Not enrolled',workingHours,workHours:workingHours,startTime,endTime,basicSalary:basicSalary||0,allowance,monthlySalary:monthlySalary || basicSalary + allowance,monthlyBonus,monthlyDeduction};
   db.employees.push(x);
